...
BEGIN
    ...
    INSERT INTO profiles (id, full_name, full_name_la, email, role)
    VALUES 
        (gen_random_uuid(), 'Dr. Somchai Vongphachan', 'ດຣ. ສົມໄຊ ວົງພະຈັນ', 'somchai.v@nuol.edu.la', 'professor')
    RETURNING id INTO author1_id;
    
    INSERT INTO profiles (id, full_name, full_name_la, email, role)
    VALUES 
        (gen_random_uuid(), 'Dr. Vilay Phonephakdy', 'ດຣ. ວິໄລ ໂພນະພັກດີ', 'vilay.p@nuol.edu.la', 'professor')
    RETURNING id INTO author2_id;
    
    INSERT INTO profiles (id, full_name, full_name_la, email, role)
    VALUES 
        (gen_random_uuid(), 'Prof. Bounmy Sisavath', 'ຜສ. ບຸນມີ ສີສະວາດ', 'bounmy.s@nuol.edu.la', 'professor')
    RETURNING id INTO author3_id;
    
    INSERT INTO profiles (id, full_name, full_name_la, email, role)
    VALUES 
        (gen_random_uuid(), 'Dr. Khamsing Keomanivong', 'ດຣ. ຄຳສິງ ແກ້ວມະນີວົງ', 'khamsing.k@nuol.edu.la', 'professor')
    RETURNING id INTO author4_id;
    
    INSERT INTO profiles (id, full_name, full_name_la, email, role)
    VALUES 
        (gen_random_uuid(), 'Prof. Phetdavanh Souvannalath', 'ຜສ. ເພັດດາວັນ ສຸວັນນະລາດ', 'phetdavanh.s@nuol.edu.la', 'professor')
    RETURNING id INTO author5_id;
    
    INSERT INTO profiles (id, full_name, full_name_la, email, role)
    VALUES 
        (gen_random_uuid(), 'Dr. Sysavath Intharath', 'ດຣ. ສີສະວາດ ອິນທະຣາດ', 'sysavath.i@nuol.edu.la', 'professor')
    RETURNING id INTO author6_id;
    
    INSERT INTO profiles (id, full_name, full_name_la, email, role)
    VALUES 
        (gen_random_uuid(), 'Prof. Latsamy Phonevilay', 'ຜສ. ລັດສະໝີ ໂພນະວິໄລ', 'latsamy.p@nuol.edu.la', 'professor')
    RETURNING id INTO author7_id;
    
    INSERT INTO profiles (id, full_name, full_name_la, email, role)
    VALUES 
        (gen_random_uuid(), 'Dr. Anousone Tampasack', 'ດຣ. ອານຸສອນ ຕຳປາສັກ', 'anousone.t@nuol.edu.la', 'professor')
